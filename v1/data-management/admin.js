...
const AdminDataManagement = function () {
    ...
    this.checkAdminExists = async (adminData, isLogin) => {
        try {
            ...
            if (_id) {
                isExists = await AdminModel.findOne({ type: userType, _id: ObjectId(_id), isDeleted: false }, Projection).lean();
            }
            else if (email) {
                isExists = await AdminModel.findOne({ type: userType, email, isDeleted: false }, Projection).lean();
            }
    ...
    this.updateAdmin = async (findByData, adminData) => {
        try {
            ...
            await AdminModel.findOneAndUpdate({ _id: ObjectId(admin._id) }, adminData);
            admin = await AdminModel.findOne({ _id: ObjectId(admin._id) }, PROJECTIONS.createAdmin).lean();
            return admin;
        }
        catch (err) {
            throw err;
            ...
            if (tokens) {
                try {
                    ...
                    let newTokens = {
                        ...
                        refreshToken: await universal.createRefreshToken({ _id: tokens.user })
                    };
                    await AuthTokenModel.findOneAndUpdate({ user: adminId }, newTokens).lean();
    ...
    this.clearTokens = async (adminId) => {
        try {
            adminId = ObjectId(adminId);
            await this.checkAdminExists({ _id: adminId });
            await AuthTokenModel.findOneAndUpdate({ user: adminId }, { token: '', refreshToken: '' }).lean();
            return;
        }
        catch (err) {
            throw err;
